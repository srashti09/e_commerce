...
const HomeSlidesList = () => {
    ...
    useEffect(() => {
        ...
        fetchDataFromApi('/api/homeBanner')
            .then((res) => {
                // Ensure res is an array before setting slideList
                if (Array.isArray(res)) {
                    setSlideList(res);
                    context.setProgress(100);
                } else {
                    console.error('Expected array of slideList, received:', res);
                }
            })
            .catch((error) => {
        ...
        deleteData(`/api/homeBanner/${id}`)
            .then(res => {
                fetchDataFromApi('/api/homeBanner')
                    ...
                    .catch((error) => {
                        console.error('Error fetching updated slideList:', error);
                    });
            })
            .catch((error) => {
